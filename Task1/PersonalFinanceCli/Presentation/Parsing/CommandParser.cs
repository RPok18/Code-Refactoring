...
public sealed class CommandParser
{
    ...
    public static int? ResolveCardFromArgs(string rawCardId)
    {
        if (int.TryParse(rawCardId, out var numericId))
        {
            return numericId;
        }

        if (Regex.IsMatch(rawCardId, "^[0-9a-fA-F-]{36}$") && Guid.TryParse(rawCardId, out var guid))
        {
